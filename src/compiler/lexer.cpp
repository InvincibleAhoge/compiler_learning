...
bool is_alpha_and_num(const char c) {
...
bool is_keyword(const char *p, const size_t len) {
    static const char *keywords[] = {
        "break", "const", "continue", "else", "float", "for", "if", "int", "return", "void", "while"
    };
    ...
    while (*p) {
        ...
        if (is_digit(*p) || (*p == '.' && is_digit(*(p + 1)))) {
            ...
            if (is_float) {
                ...
                if (*end != '\0')
                    throw std::runtime_error("invalid floating literal");
            } else {
        ...
        if (is_alpha(*p) || *p == '_') {
            ...
            while (*p && (is_alpha(*p) || *p == '_' || is_digit(*p)))
                p++;
            const auto len = static_cast<size_t>(p - start);
            const TokenKind kind = is_keyword(start, len) ? TokenKind::KEYWORD : TokenKind::IDENT;
...
static const char *token_kind_name(const TokenKind kind) {
    switch (kind) {
        ...
        case TokenKind::PUNCT: return "punctuator";
...
void print_tokens(const File &file, const std::vector<Token> &tokens) {
    ...
    for (const auto &token: tokens) {
        ...
        const auto width = std::to_string(pos.line).size() + 1 + std::to_string(pos.column).size();
    ...
    for (const auto &token: tokens) {
        ...
        const auto pos_str = std::to_string(pos.line) + ":" + std::to_string(pos.column);
        ...
        if (token.kind != TokenKind::TK_EOF) {
            std::string_view lex(file.contents.data() + token.loc.offset, token.length);
            str += '\'';
            str += lex;
            str += '\'';
        }
